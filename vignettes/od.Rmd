...
# pandoc --citeproc vignettes/od.Rmd -o vignettes/od.md --bibliography=vignettes/od.bib
# pandoc --citeproc vignettes/od.Rmd -s -t csljson -o vignettes/od.json
pandoc --filter citeproc -f markdown vignettes/od.Rmd 
pandoc --citeproc vignettes/od.Rmd -s -f markdown -t csljson
...
We will copy it into an object called `x` to be concise, and to highlight the fact that these operations can be generalised to many OD datasets.
...
Like all data, the object `x`, created in the preceding code chunk, comes from a specific context, the 2011 [UK Census](https://census.ukdataservice.ac.uk/media/50966/2011_england_household.pdf) questions:
...
`x` is an origin-destination dataset represented as a data frame containing aggregated answers to these questions (see `?pct::get_od()` for details).
...
Other columns contain attributes associated with each OD pair, typically counting how many people travel by mode of transport, as shown by printing the contents of `x`:
