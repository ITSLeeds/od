...
It is typically the job of the analyst to use a routing service (such as [OSRM](https://github.com/rCarto/osrm), [Google Directions API](https://symbolixau.github.io/googleway/articles/googleway-vignette.html#google-directions-api), [CycleStreets.net](https://github.com/Robinlovelace/cyclestreets/) or [OpenRouteService](https://github.com/GIScience/openrouteservice-r/)) or an assignment model (such as those contained in proprietary software such as [SATURN](https://saturnsoftware2.co.uk/) and [Visum](https://vision-traffic.ptvgroup.com/en-us/products/ptv-visum/functions/)) to identify likely routes with reference to shortest path algorithms or generalised cost minimisation algorithms (which account for monetary plus time and quality 'costs').
...
At its most basic, a single trip can be OD data, as illustrated in the very simple example dataset below.

## OD data representing travel from Leeds to London
...
The plot above shows that OD data is fundamentally geographic data, although it is not always represented geographically.
To convert the non-geographic OD datasets into a geographic object, we can use the `od_to_sf()` function from the `od` package as follows:

```{r}
l = od_to_sf(od_data_example, p)
l
```

We just converted the data frame into a 'geographic data frame'.
...
We will copy it into an object called `x` for conciseness and to highlight the fact that these operations can be generalised to many OD datasets.
